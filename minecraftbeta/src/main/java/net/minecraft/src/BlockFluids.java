...
public abstract class BlockFluids extends Block {
	...
	public void randomDisplayTick(World world, int i, int j, int k, EaglercraftRandom random) {
		if (blockMaterial == Material.water && random.nextInt(64) == 0) {
			...
			if (l > 0 && l < 8) {
				world.playSoundEffect((float) i + 0.5F, (float) j + 0.5F, (float) k + 0.5F, "liquid.water",
						random.nextFloat() * 0.25F + 0.75F, random.nextFloat() * 1.0F + 0.5F);
			}
		}
		if (blockMaterial == Material.lava && world.getBlockMaterial(i, j + 1, k) == Material.air
				&& !world.isBlockOpaqueCube(i, j + 1, k) && random.nextInt(100) == 0) {
	...
	protected void func_292_i(World world, int i, int j, int k) {
		...
		for (int l = 0; l < 8; l++) {
			world.spawnParticle("largesmoke", (double) i + Math.random(), (double) j + 1.2D, (double) k + Math.random(),
					0.0D, 0.0D, 0.0D);
		}
