...
public class StreamThread extends SimpleThread {
	 ...
	 * state to "not alive". Method alive() returns false when the cleanup() method
	 * has completed.
	 */
	@Override
			...
			while (!dying() && !streamingSources.isEmpty()) {
				...
				synchronized (listLock) {
					...
					while (!dying() && iter.hasNext()) {
						src = iter.next();
						// If this is a removed source, we cleanup here and then let normal cleanup run
						...
						if (src == null) {
							...
							if ((!src.stream()) && (!src.rawDataStream)) {
								if (src.channel == null || !src.channel.processBuffer()) {
									...
									/*
									 * if( src.getSoundSequenceQueueSize() > 0 ) { src.incrementSoundSequence(); }
									 * 
									 * // check if this is a looping source else
									 */ if (src.toLoop) {
										// wait for stream to finish playing
										if (!src.playing()) {
											// Generate an EOS event:
											SoundSystemConfig.notifyEOS(src.sourcename,
	 ...
	 * already playing on the same channel, it is stopped and removed from the list.
	 * 
