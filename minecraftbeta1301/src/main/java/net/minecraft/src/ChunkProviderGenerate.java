...
public class ChunkProviderGenerate implements IChunkProvider {
	...
	public void populate(IChunkProvider ichunkprovider, int i, int j) {
		...
		int k = i * 16;
		int l = j * 16;
		MobSpawnerBase mobspawnerbase = worldObj.getWorldChunkManager().func_4073_a(k + 16, l + 16);
		rand.setSeed(worldObj.func_22138_q());
		long l1 = (rand.nextLong() / 2L) * 2L + 1L;
		long l2 = (rand.nextLong() / 2L) * 2L + 1L;
		rand.setSeed((long) i * l1 + (long) j * l2 ^ worldObj.func_22138_q());
		double d = 0.25D;
		if (rand.nextInt(4) == 0) {
		...
		d = 0.5D;
		int k4 = (int) ((mobSpawnerNoise.func_806_a((double) k * d, (double) l * d) / 8D + rand.nextDouble() * 4D + 4D)
				/ 3D);
		int l7 = 0;
