...
public class MapGenCavesHell extends MapGenBase {
	...
	protected void func_4129_a(int i, int j, byte abyte0[], double d, double d1, double d2) {
		func_4128_a(i, j, abyte0, d, d1, d2, 1.0F + rand.nextFloat() * 6F, 0.0F, 0.0F, -1, -1, 0.5D);
	}

	protected void func_4128_a(int i, int j, byte abyte0[], double d, double d1, double d2, float f, float f1, float f2,
		...
		for (; k < l; k++) {
			...
			if (!flag && k == j1 && f > 1.0F) {
				func_4128_a(i, j, abyte0, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - 1.570796F, f2 / 3F, k, l,
						1.0D);
				func_4128_a(i, j, abyte0, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + 1.570796F, f2 / 3F, k, l,
						1.0D);
				return;
				...
				for (int k3 = (int) d10; k3 < i2; k3++) {
					...
					for (int i4 = l1 - 1; i4 >= d9; i4--) {
						...
						if (d14 > -0.69999999999999996D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D) {
							...
							if (byte0 == Block.bloodStone.blockID || byte0 == Block.dirt.blockID
